...
class Num:
    def __init__(self, first_item, col_name, discretization=None):
        self.mu = 0
        self.n = 0
        self.m2 = 0
        self.up = float("-inf")
        ...
        if discretization is not None:
            self.frequency_bins = [] # N-1 bin dividers
        self.add(first_item)
    ...
    def eid(self, feature_value):
        """
        Returns the equal interval discretization value.
        Bins computed with (high - lo) / bins
        """
        return int((self.discretization.bins-1) * (feature_value - self.lo) / (self.up - self.lo))

    def recompute_counts(self, discretized_value):
        self.counts[discretized_value] = self.counts.get(discretized_value, 0) + 1
        self.n += 1

    def recompute_frequency_bins(self, col_values):
        """
        Recompute the frequency bins for this column.
        """
        #print "Recomputing frequency bins for column ", self.name
        self.frequency_bins = []
        col_values = sorted(col_values)
        bin_size = int(math.ceil(len(col_values) / self.discretization.bins))
        for i in xrange(1, self.discretization.bins):
            self.frequency_bins.append(col_values[i * bin_size])

    def efd(self, feature_value):
        result = self.discretization.bins-1
        # Using a linear search. Switch to binary if too slow
        for index, bin_upper_bound in enumerate(self.frequency_bins):
...
class Table:
    # Constructor
    def __init__(self, filename=None, discretization=None):
        """
    ...
    def populate(self, file_reader):
        ...
        self.output_rows.append(self.rows[-1])
        col_features = map(self.construct_feature_column, enumerate(zip(features, header[:-1])))
        col_outcomes = map(Table.construct_outcome_column, zip(outcomes, header[-1:]))
    ...
    def add_row(self, row):
        ...
        if self.cols is None:
            col_features = map(self.construct_feature_column, enumerate(itertools.izip_longest(row.features, [])))
            col_outcomes = map(Table.construct_outcome_column, itertools.izip_longest(row.outcomes, []))
            self.cols = Row(col_features, col_outcomes)
        else:
            for item, col in zip(row, self.cols):
                col.add(item)

    def construct_feature_column(self, (index, (item, name))):
        """
        Takes an item and constructs the appropriate column type.
        """
        try:
            col = Num(item, name, self.discretization)
            self.numerical_cols[index] = col
    ...
    @staticmethod
    def construct_outcome_column((item, name)):
        """
        Takes an item and constructs the appropriate column type.
        """
        try:
            col = Num(item, name, None)
        except:
            col = Sym(item, name)
        return col
    ...
    def iterate_rows(self, features_only=True):
        #print "Calling iterate_rows. discretization: ", self.discretization
        if self.discretization is not None and not self.updated_output:
            self.recompute_output_rows()
        if features_only:
    ...
    def recompute_frequency_bins(self):
        """
        Recomputes the EFD bins.
        """
        #print "Entering recompute_frequency_bins"
        # Find the relevant columns
        recompute_rows = {} # col_name, feature_values
        for index in self.numerical_cols:
            recompute_rows[index] = (self.numerical_cols[index], [])

        # Pull out these columns' feature values
        for row in self.rows:
            for index in recompute_rows:
                recompute_rows[index][1].append(row.features[index])

        #print recompute_rows
        # Update the frequency bins
        for index in recompute_rows:
    ...
    def recompute_output_rows(self):
        ...
        self.updated_output = True
        self.output_rows = []

        if self.discretization.method == "EID":
            for row in self.rows:
                new_features = []
                for col, feature in zip(self.iterate_cols(features_only=True), row.features):
                    #print "Looking at col ", col.name, " of type ", col.type
                    if col.type == "Num":
                        new_features.append(col.eid(feature))
                        col.recompute_counts(new_features[-1])
                    else:
                        new_features.append(feature)
                self.output_rows.append(Row(new_features, row.outcomes))
            ###
            #print "New output rows: "
            #for output_row, row in zip(self.output_rows, self.rows):
            #    print output_row.features, row.features
            ###

        elif self.discretization.method == "EFD":
            self.recompute_frequency_bins()
            for row in self.rows:
                new_features = []
                for col, feature in zip(self.iterate_cols(features_only=True), row.features):
                    if col.type == "Num":
